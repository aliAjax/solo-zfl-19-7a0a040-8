...
// 路径中的机台编号是唯一归属：请求体若带 machineId（id 或 code 均可），
// 必须解析到路径所指的同一台机台，否则返回明确的 400 校验错误，
// 防止样本/维修记录写到另一台机台。
function assertPathOwnsMachine(data, pathId, bodyMachineId) {
  const pathMachine = getMachineOrFail(data, pathId);
  if (bodyMachineId === undefined || bodyMachineId === null) return pathMachine;
  const bodyMachine = data.machines.find((m) => m.id === bodyMachineId || m.code === bodyMachineId);
  if (!bodyMachine || bodyMachine.id !== pathMachine.id) {
    throw httpError(400, "请求体中的机台编号与请求路径不一致");
  }
  return pathMachine;
}
...
function ingestSamples(data, body, clock) {
  ...
  // 收到上报即恢复在线：掉线判定统一走 effectiveStatus，并把结果一并持久化，
  // 保证机台状态与本次批次/故障在同一次原子写入中落库。
  const nextStatus = effectiveStatus(machine, now);
